                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group input-focused">
                                            ...
                                            <input type="text" name="pincode" id="pincode" value="000000" class="form-control" value="{{ old('pincode') }}">
                                        ...
                                        <div class="form-group input-focused">
                                            ...
                                            <select class="form-select" id="guest_checkout_country" name="emirate" data-error-container="#guest-emirate-error">
                                                ...
                                                @foreach($countries as $country)]
                                                    @if($country->getName() == 'UAE' || $country->getName() == 'Uae' || $country->getName() == 'الإمارات العربية المتحدة')
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group input-focused">
                                            <label>City</label>
                                            <select class="form-select" id="guest_checkout_city" name="city" data-error-container="#city-error"> 
                                                    ...
                                                    <li>
                                                        <div class="cart-quantity">
                                                            ...
                                                            <input type="number" class="product-qty" name="quantity" min="{{ $product->minimum_qty }}"
                                                             max="{{ $product->total_stock }}" value="{{ $product->quantity }}" data-error-container="#quantity-error" style="width: 100px;" readonly="true">                                                                                                        
                             ...
                             <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group input-focused">
                                        ...
                                        <input type="text" id="delivery_date" name="delivery_date" class="form-control datepicker">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group input-focused">
                                        ...
                                        <select class="form-select" id="delivery_time" name="delivery_time" data-error-container="#delivery_time-error">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group input-focused desc" data-char="0/1000">
                                        ...
                                        <textarea class="form-control" id="delivery_comment" name="delivery_comment" placeholder="Delivery Message" rows="10"></textarea>
                    ...
                    <div class="order-summary">
                            <div class="order-summary-inner">
                                ...
                                <table width="100%">
                                    ...
                                    <tr>
                                        <td>Delivery</td>
                                        <td id="charges">AED {{ $delivery_charge }}</td>
                                    </tr>
                                    <tr>
                                        <td>VAT</td>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        window.onbeforeunload = function() {
            ...
            localStorage.setItem("guest_checkout_country", $("#guest_checkout_country").val());
            localStorage.setItem("guest_checkout_city", $("#guest_checkout_city").val());
            localStorage.setItem("delivery_date", $("#delivery_date").val());
            localStorage.setItem("delivery_time", $("#delivery_time").val());
            localStorage.setItem("delivery_comment", $("#delivery_comment").val());
        }
        ...
        window.onload = function() {
            ...
            var guest_checkout_country  = localStorage.getItem("guest_checkout_country");
            var guest_checkout_city     = localStorage.getItem("guest_checkout_city");
            var delivery_date           = localStorage.getItem("delivery_date");
            var delivery_time           = localStorage.getItem("delivery_time");
            var delivery_comment        = localStorage.getItem("delivery_comment");

            // Set Values To Fields
            if (first_name !== null){
                $('#first_name').val(first_name);
                $('#first_name').parent('div').addClass('input-focused');
            }
            if (last_name !== null){
                $('#last_name').val(last_name);
                $('#last_name').parent('div').addClass('input-focused');
            } 
            if (address !== null){
                $('#address').val(address);
                $('#address').parent('div').addClass('input-focused');
            }
            if (contact !== null){
                $('#contact').val(contact);
                $('#contact').parent('div').addClass('input-focused');
            }
            if (email !== null){
                $('#email').val(email);
                $('#email').parent('div').addClass('input-focused');
            }
            if (delivery_time !== null){
                $('#delivery_time').val(delivery_time);
                $('#delivery_time').parent('div').addClass('input-focused');
            }
            if (pincode !== null) $('#pincode').val(pincode);
            if (guest_checkout_country !== null) $('#guest_checkout_country').val(guest_checkout_country);
            if (guest_checkout_city !== null) $('#guest_checkout_city').val(guest_checkout_city);
            if (delivery_date !== null) $('#delivery_date').val(delivery_date);
            if (delivery_comment !== null) $('#delivery_comment').val(delivery_comment);
            ...
            $.ajax({
                ...
                success: function(Response){
                    if(Response.success){
                        ...
                        $("#guest_checkout_city").empty();
                        $("#guest_checkout_city").append('<option value="" selected>SELECT CITY</option>');

                        $(cities).map((id,city) => {
                            $("#guest_checkout_city").append('<option value='+city['id']+'>'+city['city_name']+'</option>');
                        });
                    }
                    if(Response.fail){
                        $("#guest_checkout_city").empty();
        ...
        $('#guest_checkout_city').on('change',function(){
            var element    = $(this);
            var city       = $(this).val();
              $.ajax({
                        type: 'POST',
                ...
                $('.error-help').remove();
                    $.ajax({
                        ...
                        data: {
                            ...
                            city: function(){
                                return $("#guest_checkout_city").val();
                            },
                            pincode: function(){
                                return $("#pincode").val();
                            },
