			            ...
			            <div class="col-md-12">
			                <div class="portlet light ">
			                    ...
			                    <div class="portlet-body">
	                                <form action="{{ route('admin.roles.store') }}" role="form" id="frmAddNewRole" name="frmAddNewRole" method="POST" enctype="multipart/form-data">
	                                	@csrf

                                        {{-- First Name --}}
	                                    <div class="form-group {{ $errors->has('full_name') ? 'has-error' : '' }}">
	                                        <label class="control-label">{!! $mend_sign !!}Name</label>
	                                        <input type="text" placeholder="Enter Name Here" class="form-control" id="full_name" name="full_name" maxlength="50" autocomplete="off" />
                                                            ...
                                                            <tr class="role_row_{{ $role->id }}">
                                                                ...
                                                                <td>
                                                                    @if ( in_array('access', $allowed_permissions) )
                                                                        <input data-role-id="{{ $role->id }}" class="role_permission_{{ $role->id }}" data-error-container='#permission_error' name="roles[{{ strtolower($role->id) }}][permissions][]" type="checkbox" value="access" {{ $role->id == 1 ? 'checked disabled' : '' }}>
                                                                    @else N / A @endif
                                                                </td>
                                                                <td>
                                                                    @if ( in_array('add', $allowed_permissions) )
                                                                        <input data-role-id="{{ $role->id }}" class="role_permission_{{ $role->id }}" data-error-container='#permission_error' name="roles[{{ strtolower($role->id) }}][permissions][]" type="checkbox" value="add">
                                                                    @else N / A @endif
                                                                </td>
                                                                <td>
                                                                    @if ( in_array('edit', $allowed_permissions) )
                                                                        <input data-role-id="{{ $role->id }}" class="role_permission_{{ $role->id }}" data-error-container='#permission_error' name="roles[{{ strtolower($role->id) }}][permissions][]" type="checkbox" value="edit">
                                                                    @else N / A @endif
                                                                </td>
                                                                <td>
                                                                    @if ( in_array('delete', $allowed_permissions) )
                                                                        <input data-role-id="{{ $role->id }}" class="role_permission_{{ $role->id }}" data-error-container='#permission_error' name="roles[{{ strtolower($role->id) }}][permissions][]" type="checkbox" value="delete">
                                                                    @else N / A @endif
                                                                </td>
                                                                <td>
                                                                    @if ( in_array('view', $allowed_permissions) )
                                                                        <input data-role-id="{{ $role->id }}" class="role_permission_{{ $role->id }}" data-error-container='#permission_error' name="roles[{{ strtolower($role->id) }}][permissions][]" type="checkbox" value="view">
                                                                    @else N / A @endif
                                                                </td>
                                                                <td><input type="checkbox" class="checkrow"  name="checkrow" data-roleid='{{ $role->id }}'></td>
                                                            </tr>
                                                        @endif
                                                    @endforeach
                                                </tbody>
